...
class Djikstra {
    ...
    public static void solver() {
        ...
        for(int i = 0; i <= vtcs; i++){
            graph.add(new ArrayList<>());//one based indexing hai issiliye <=vtcs kiya hai actualy 0 idx is useless
        }
        ...
        pq.add(new Pair (0,0));//0,0 add nahi karenge as 1 based indexing hai issiliye 1,0 add karnege
        ...
        while(pq.size() > 0){
            ...
            visited[rem.vtx] = rem.wt;


            //add*
            ArrayList<Pair> nbrs = graph.get(rem.vtx);
            for(Pair nbr : nbrs){
                if(visited[nbr.vtx] != -1){
                    continue;
                }
