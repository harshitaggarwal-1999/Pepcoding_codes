...
public class mstWIthPath {
    ...
    public static void solver() {
        ...
        while (pq.size() > 0) {
            ...
            if (vis[currpair.destvtx] == true)
                continue;

            vis[currpair.destvtx] = true;

            mst.get(currpair.srcvtx).add(new pair(currpair.srcvtx, currpair.destvtx, currpair.wt));
            
            ans += currpair.wt;

            for(pair nbr : graph.get(currpair.destvtx)){
                if(vis[nbr.destvtx] == false){
                    pq.add(nbr);
                }
            }
        ...
        for(ArrayList<pair> arr: mst){
            for(pair p : arr){
                System.out.println(p.srcvtx+ " "+ p.destvtx + " "+ p.wt);
            }
        }
    ...
    static class pair implements Comparable<pair> {
        int srcvtx, destvtx, wt;

        public pair(int srcvtx, int destvtx, int second) {
            this.srcvtx = srcvtx;
