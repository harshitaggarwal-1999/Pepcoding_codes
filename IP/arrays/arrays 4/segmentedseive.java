...
public class segmentedseive{
    ...
    public static void solver() {
        ...
        List<Long> ans = segmentedSeive(m, n);
        System.out.println("ans: "+ ans);

    }

    public static List<Long> segmentedSeive(long m, long n){
        boolean[] arr = new boolean[(int)(n-m)+1];

        Arrays.fill(arr, true);
        // false -> notprime, true ->  prime
        for(int i = 0; i <= Math.sqrt(n); i++){
            if(arr[i] == false){
                continue;
            }else{
                if(isPrime((long)i+m) == true){
        ...
        for(int i = 2; i <= n-m; i++){
            if(arr[i] == true)ans.add((long)i+m);
        }
        return ans;
    }
