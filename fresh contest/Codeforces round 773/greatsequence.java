    ...
    public static void solver() {
        ...
        for(int i = 0; i < n; i++){
            ...
            if(hm.containsKey(arr[i])){
                long tempnum = hm.get(arr[i]);
                tempnum++;
                hm.put(arr[i],tempnum);
            }else{
                hm.put(arr[i],(long)1);
            }
        }
        ...
        for(int i = 0; i < n; i++){
            long currnum = arr[i];
            long currnummul = arr[i]*x;
            if(hm.containsKey(currnummul) && hm.containsKey(currnum)){
                long currnummulval = hm.get(currnummul);
                currnummulval--;
                long currnumval = hm.get(currnum);
                currnumval--;

                if(currnummulval == 0){
                    hm.remove(currnummul);
                }else{
                    hm.put(currnummul, currnummulval);
                }
                if(currnumval == 0){
                    hm.remove(currnum);
                }else{
                    hm.put(currnum, currnumval);
                }
