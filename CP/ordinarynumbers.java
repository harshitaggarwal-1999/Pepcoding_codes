...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>> CODE STARTS HERE <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    public static void solver() {
        ...
        if(n< 11)System.out.println(9);
        // if(n == 11)
        int prevnum = 11;
        int count = 0;
        int checkpoint = 100;
        int num = 11;
        for(int i = 0; i < n && n>=11 && num <= n; i++){
            // System.out.println(num+" "+ n+ " "+ prevnum+" "+checkpoint);
            if(num<checkpoint){num += prevnum;count++;}
            else{
                num = checkpoint+prevnum;
                checkpoint *= 10;
                prevnum = num;
            }
        }
