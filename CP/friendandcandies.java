...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>LOWER BOUND<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    public static int LowerBound(int arr[], int x) { // x is the target value or key
...
//>>>>>>>>>>>>>>>>>>>>>>>>>> SWAP <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    public static void solver() {
        ...
        if(sum %num != 0){System.out.println(-1);return;}

        int kilo = sum/num;
        Arrays.sort(array);
        
        System.out.println(num-(LowerBound(array,kilo+1)));
    }
}