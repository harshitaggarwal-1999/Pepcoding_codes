...
public class permutationtransformation {
    ...
    public static void depth(int l, int r,int[] arr, int[] darr, int currdepth){
        if(r < l){
            return;
        }

        if(l == r){
            darr[l] = currdepth;return;
        }
        ...
        depth(l, m-1, arr, darr, currdepth+1);
        depth(m+1, r, arr, darr, currdepth+1);
    }
    //-------------------------------- HO JA BHAI ----------------------------------------------------
    /* code ends here*/
