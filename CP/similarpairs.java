...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>> CODE STARTS HERE <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    public static void solver() {
        ...
        for(int i = 0 ; i < n; i++){
            if(arr[i]%2 == 0 && even.containsKey(arr[i])==false){
                even.put(arr[i],1);
            }
            if(arr[i]%2 != 0){
                a.add(arr[i]);
        ...
        for(int i: a){
            if(even.containsKey(i+1) || even.containsKey(i-1)){System.out.println("YES");return;}
        }
        if(total%2){System.out.println("NO");return;}
