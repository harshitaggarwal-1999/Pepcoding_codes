...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>LOWER BOUND<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    public static long modular_add(long a, long b){
	    return ((a % mod) + (b % mod)) % mod;
	}
	public static long modular_sub(long a, long b){
	    return ((a % mod) - (b % mod) + mod) % mod;
	}
	public static long modular_mult(long a, long b){
	    return ((a % mod) * (b % mod)) % mod;
	}
...
//>>>>>>>>>>>>>>>>>>>>>>>>>> SWAP <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    public static int sumofdig(int m){
        int sum = 0;
        while(m > 0)
        {
            int n = m % 10;
