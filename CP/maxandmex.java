...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>> CODE STARTS HERE <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    public static void solver() {
        ...
        int max = MIN;
        int min = MAX;
        ArrayList<Integer> missingelem = new ArrayList<>();
        ...
        if(k <= 0){
            System.out.println(mymap.size());return;
        }
        int mexmin = 0;
        for(int i = 0 ; i <= max; i++){
        ...
        // System.out.println("mex:" + mexmin + " max: " + max);
        // System.out.println(mymap);

        if(mexmin<max){
            int numcalculated =(int) Math.ceil((double)(max+mexmin)/2.0);
            if (mymap.containsKey(numcalculated)){
            ...
            System.out.println(mymap.size()+k);
        }
